...
def remove_template_entry(template_id, existdb=None):
    ...
    if row is not None:
        ...
        raise errors.ToasterError("Cannot remove template (ID: %d). " \
                "It has been been used for processing." % template_id)
    else:
...
def main(args):
    ...
    trans = db.begin()
    try:
        template = utils.get_template_from_id(args.template_id, existdb=db)
        ...
        if args.action=='leave':
            ...
            utils.print_info("Moving template %s to %s" % (template, args.dest))
            shutil.move(template, args.dest)
        elif args.action=='delete':
            utils.print_info("Deleting template %s" % template)
            os.remove(template)
        else:
