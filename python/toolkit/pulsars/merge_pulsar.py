...
def merge_pulsar(src_pulsar_id, dest_pulsar_id, existdb=None):
    """Merge one pulsar entry into another.
        ...
        Outputs:
            None
    """
    utils.print_info("Merging pulsar '%s' (ID: %d) into '%s' (ID: %d)" % \
                    (utils.get_pulsarname(src_pulsar_id), src_pulsar_id, \
                     utils.get_pulsarname(dest_pulsar_id), dest_pulsar_id), 2)
...
def main(args):
    src_pulsar_id = utils.get_pulsarid(args.src_psrname)
    dest_pulsar_id = utils.get_pulsarid(args.dest_psrname)
    if src_pulsar_id == dest_pulsar_id:
        raise errors.BadInputError("Cannot merge '%s' (ID: %d) into " \
                    "itself ('%s')" % \
                    (args.src_psrname, src_pulsar_id, args.dest_psrname))
