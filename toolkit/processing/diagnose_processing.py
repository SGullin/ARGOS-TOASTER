...
def check_processing_diagnostic_existence(proc_id, diagname, existdb=None):
    ...
    if len(floatrows) > 1:
        raise errors.InconsistentDatabaseError("There should be no "
                                               "more than one float "
                                               "diagnostic of each type "
                                               "per processing job. There "
                                               "are %d '%s' diagnostic "
                                               "values for this processing "
                                               "job (ID: %d)" %
                                               (len(floatrows), diagname,
                                                proc_id))
    elif len(floatrows) == 1:
        val = floatrows[0]['value']
        raise errors.DiagnosticAlreadyExists("There is already a '%s' float "
                                             "diagnostic value in the DB for "
                                             "this processing job "
                                             "(ID: %d; value: %g)" %
                                             (diagname, proc_id, val))
...
def __insert_processing_float_diagnostic(proc_id, diag, existdb=None):
    ...
    try:
        ...
        result.close()
        notify.print_info("Inserted process diagnostic (type: %s)." %
                          diag.name, 2)
    finally:
        if not existdb:
            # Close DB connection
...
def __insert_processing_diagnostic_plot(proc_id, diag, archivedir=None,
    ...
    try:
        ...
        if suffix:
            ...
            if os.path.dirname(newfn) != os.path.dirname(origfn):
                raise errors.FileError("Adding processing diagnostic "
                                       "plot suffix will cause plot to "
                                       "be moved to annother directory "
                                       "(new file name: %s)!" % newfn)
        ...
        diag_id = result.inserted_primary_key[0]
        result.close()
        notify.print_info("Inserted process diagnostic plot (type: %s)." %
                          diag.name, 2)
    except:
