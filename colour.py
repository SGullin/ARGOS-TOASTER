...
USECOLOURS = 'NO_TOASTER_COLOURS' not in os.environ
    ...
    if preset is not None:
        ...
        for a in attr.keys():
            if a in attributes:
                ...
                raise ValueError("Unrecognized attribute:", a)
        ...
        elif isinstance(fg, str) or fg.isdigit():
            fg_val = str(fg)
        else:
            raise ValueError("Unrecognized foreground colour:", fg)
            
        if bg in bg_colours:
            bg_val = bg_colours[bg]
        elif isinstance(bg, str) or bg.isdigit():
            bg_val = str(bg)
        else:
            raise ValueError("Unrecognized background colour:", bg)
...
def parse_attributes(option, opt_str, value, parserobj):
    """Parse text attributes from command line.
    """
    if not hasattr(parserobj.values, 'attributes'):
        # Create empty dictionary for text attributes
        setattr(parserobj.values, 'attributes', {})
    parserobj.values.attributes[value] = True
...
if __name__ == '__main__':
    ...
    parser.add_option('-s', '--set', dest='toset', type='string',
                      action='callback', callback=parse_attributes,
                      help="Set text attributes. Possible attributes to "
                           "set are: defaut, bold, dim, underline, blink, "
                           "reverse and hidden.")
    parser.add_option('-f', '--fg', dest='fg', action='store',
                      help='Forground text colour.', default='default')
    parser.add_option('-b', '--bg', dest='bg', action='store',
                      help='Background text colour.', default='default')
    parser.add_option('-p', '--preset', dest='preset', action='store',
                      help='Use a preset colour scheme. Other options will be ignored.', default=None)
